...
//
//    private static final List<String> openApiEndpoints = Arrays.asList("/auth/login", "/auth/register");
//
//    @Override
...
//            LOGGER.info("JWT claims extracted: id={}, role={}, username={}, email={}", claims.get("id"), claims.get("role"), claims.get("username"), claims.get("email"));
//
//            ServerHttpRequest mutatedRequest = exchange.getRequest().mutate().header("id", String.valueOf(claims.get("id"))).header("role", String.valueOf(claims.get("role"))).header("username", String.valueOf(claims.get("username"))).header("email", String.valueOf(claims.get("email"))).build();
//
//            ServerWebExchange mutatedExchange = exchange.mutate().request(mutatedRequest).build();
//            return chain.filter(mutatedExchange);
//        } catch (Exception e) {
//            LOGGER.error("Error processing the JWT token for request : {}", e.getMessage());
...
//
//        String errorResponse = new JSONObject(Map.of("status", HttpStatus.FORBIDDEN.name(), "statusCode", HttpStatus.FORBIDDEN.value(), "statusMessage", HttpStatus.FORBIDDEN.getReasonPhrase(), "errorMessage", err)).toString();
